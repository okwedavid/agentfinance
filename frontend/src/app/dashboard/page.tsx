...
function TaskCard({
  ...
  return (
    <div className="rounded-[24px] border border-white/8 bg-white/[0.03] p-4 transition hover:border-white/15">
      ...
      {result && (
        <>
          <button onClick={() => setExpanded((value) => !value)} className="mt-4 text-sm text-cyan-200 transition hover:text-white">
            {expanded ? "Hide output" : "Show output"}
          </button>
          {expanded && (
            <pre className="mt-3 max-h-64 overflow-auto rounded-[20px] border border-white/8 bg-black/20 p-4 text-xs leading-6 text-slate-300">
              {result}
            </pre>
          )}
        </>
              ...
              <div className="mt-4 space-y-3">
                {recentTasks.length === 0 ? (
                  ...
                  recentTasks.map((task) => (
                    <TaskCard key={task.id} task={task} onDelete={removeTaskFromHistory} />
                  ))
                )}
              </div>
            </div>
